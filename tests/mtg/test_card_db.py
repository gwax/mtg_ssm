...
def test_load_mtgjson(sets_data):
    ...
    setname_to_setcode = {n: s.code for n, s in cdb.setname_to_card_set.items()}
...
def test_rebuild_indexes(sets_data):
    ...
    for snnm_key in snnm_keys:
        found_print_ids = {p.id_ for p in cdb.set_name_num_mv_to_printings[snnm_key]}
...
def test_index_sorting(sets_data):
    ...
    forest_set_and_mvid = [
        (p.set_code, p.multiverseid) for p in cdb.card_name_to_printings["Forest"]
    ]
